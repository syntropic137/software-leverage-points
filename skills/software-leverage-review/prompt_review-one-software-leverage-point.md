...
6. Emit a single YAML document conforming to `SLP_OUTPUT_SCHEMA_PATH` (Stage 1: Per-SLP output, per the policy). No sectioning, no `action` field, no markdown around the YAML, no prose.

## Output

Return ONLY the YAML document. No prose, no markdown fence. The orchestrator parses your output mechanically.
